...
from .forms import DoctorRegisterForm,DoctorUpdateForm, AdminRegisterForm,AdminUpdateForm, PatientRegisterForm,PatientUpdateForm,PatientAppointmentForm,AdminAppointmentForm,YourHealthEditForm,AppointmentEditForm,AdmitRegisterForm,AdminAdmitRegisterForm,AddMedForm,OpcostsForm,CovidVaccinationApplicationForm, WorkerRegisterForm
...
json_path = os.path.join(file_path, 'doctor.json')
    ...
    if check_admin(request.user):
        if request.method=="POST":  #if form is submitted
            ...
            if appointmentForm.is_valid():
                ...
                hospital = Admin.objects.get(user = request.user)
                if check_avail(doc,appointmentForm.cleaned_data.get('calldate'),appointmentForm.cleaned_data.get('calltime'), hospital):  #check if appointment is available during that slot
...
def patient_all_view(request):
    if check_admin(request.user):
        ...
        det=[]
        hospital = Admin.objects.get(user = request.user)
        patients = Patient.objects.filter(PatientApp__hospital=hospital)
        for c in patients:
            det.append([c.user,c.dob,c.address,c.city,c.country,c.postalcode])
...
def doctor_all_view(request):
    ...
    if check_admin(request.user):
        det=[]
        hospital = Admin.objects.filter(user = request.user).first()
        with open(json_path, 'r') as rfile:
            data = rfile.read()
            d_data = json.loads(data)
            # print(hospital.id)
            doctors = d_data[str(hospital.id)] if str(hospital.id) in d_data else []
        for c in Doctor.objects.filter(status=True, id__in = doctors).all():
...
def approve_appoint_view(request):
    if check_admin(request.user):
        ...
        det=[]
        hospital = Admin.objects.filter(user = request.user).first()
        for c in Appointment.objects.filter(status=False, hospital = hospital).all():
            d=c.doctor
            p=c.patient
            if d and p:
...
def check_avail(doc,dt,tm, hsptl):
    k = Appointment.objects.filter(doctor=doc,calldate=dt, calltime=tm, hospital = hsptl)
    if k:
        return False
    return True
    ...
    if check_patient(request.user):
        ...
        if request.method=="POST":  #if patient books an appointment
            ...
            if appointmentForm.is_valid():  #if form is valid
                ...
                hsptl_id = request.POST.get("hospital")
                print("HELLO", hsptl_id)
                hospital = Admin.objects.get(id=hsptl_id)
                if check_avail(doc,appointmentForm.cleaned_data.get('calldate'),appointmentForm.cleaned_data.get('calltime'), hospital):  #check if doctor is available during that slor
                    ...
                    appointmentForm.add_error('calltime', 'Slot Unavailable.')
                return render(request,'hospital/Patient/bookapp.html',{'appointmentForm': appointmentForm,'p1':app_det, 'total_days' : total_days, 'hospitals' : hospitals})
...
def admit_details_view(request):
    if check_patient(request.user):
        #get information from database and render in html webpage
        pat=Patient.objects.get(user_id=request.user.id)
        appointments = Appointment.objects.filter(patient=pat,finished = True)
        return render(request,'hospital/Patient/admit_details.html',{'app':appointments})
    else:
        auth.logout(request)
        return redirect('login_pat.html')
    ...
    if request.method=="POST":
        ...
        if form.is_valid(): #if form is valid
            ...
            if db < timezone.now().date():  #check if date is valid
                ...
                p.save()
                path = PatHealth(patient=p,status=True, height = form.cleaned_data.get('height'), weight = form.cleaned_data.get('weight'))    #create patient health instance for newly registered patient
                path.save()
                mpg = Group.objects.get_or_create(name='PATIENT')   #add user to patient group
                mpg[0].user_set.add(nu)
                return redirect('patient_adm.html')
...
def edityourhealth_view(request):
    if check_patient(request.user):
        ...
        if request.method=="POST":
            ...
            # p_form.fields['name'].initial = info.patient.name
            context = {
                'p_form': p_form,
                'info':info,
                'pat':pat,
        ...
        if form.is_valid(): #if form is valid
            ...
            if db < timezone.now().date():  #if date of birth is valid
                ...
                with open(json_path, 'r') as rfile:
                    ...
                    if hospital_id in parsed_data:
                        parsed_data[hospital_id].append(dp.id)
                        # print("HERE", )
                    
                    
                    # parsed_data[hospital_id] = [dp.id]
                ...
                form.add_error('dob', 'Invalid date of birth.')
                return render(request,'hospital/Doctor/register_doc.html',{'form': form, 'hospitals' : hospitals})
        else:
            print(form.errors)
    else: 
...
def register_worker(request):
    ...
    return render(request,'hospital/Worker/register.html',{'form': form})
...
def login_worker(request):
    if request.method=="POST":
        ...
        return render(request, 'hospital/Worker/login.html', {'form': form})
    else: 
        form = AuthenticationForm()
                
    return render(request, 'hospital/Worker/login.html', {'form': form})

@login_required(login_url='login_worker')
def home_worker(request):
    form = AdminAppointmentForm()
    user = request.user
    worker = models.Worker.objects.get(user = user)
    village = worker.village
    users = Patient.objects.filter(address = village)
    hospitals = Admin.objects.all()
    if request.method =='POST':
        patient = Patient.objects.get(id=request.POST.get("user"))
        hospital = Admin.objects.get(id=request.POST.get("hospital"))
        description = request.POST.get('description')
        calldate = request.POST.get('calldate_year')+"-"+ request.POST.get('calldate_month')+"-"+request.POST.get("calldate_day")
        calltime = request.POST.get('calltime')
        doctor = Doctor.objects.get(id=request.POST.get('doctor'))

        appointment = Appointment(patient = patient, doctor = doctor, description = description, hospital = hospital, calldate = calldate, calltime = calltime)
        appointment.save()
    ...
    return render(request, 'hospital/Worker/dashboard.html', {'appointmentForm' : form, 'hospitals' : hospitals, 'users' : users})




def doctor_confirmed(request, pk):
    message_content = f"Vaccination completed successfully."
...
def get_doctors(request):
    hospital_id = request.GET.get('pk')
    hospital = Admin.objects.filter(id = hospital_id).first()
    with open(json_path, 'r') as rfile:
            data = rfile.read()
            d_data = json.loads(data)
            doctors_id = d_data[str(hospital.id)] if str(hospital.id) in d_data else []
            doctors = Doctor.objects.filter(status=True, id__in = doctors_id).all()
            res = []
